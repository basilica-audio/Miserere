...
- **A factory-preset headroom gate** (`tests/PresetHeadroomTests.cpp`). Every shipped factory
  ...
  **Nothing needed fixing.** All 13 presets already pass on both paths, at −5.62 to
  −11.14 dBFS on session load (worst *BV Mode* at −5.62 dBFS) and no worse
  than −5.51 dBFS on recall; the departure state renders at −9.81 dBFS. No
