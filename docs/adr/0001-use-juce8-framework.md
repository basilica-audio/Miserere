...
Miserere needs a cross-platform audio plugin framework that can target AU and VST3 on macOS and VST3 on Windows, ship a Standalone build, and support a modern C++ toolchain. Which framework should the project build on?
...
* Breadth of built-in DSP coverage (filters, oversampling, convolution, dynamics, etc.) so the parallel bus routing, passive-style EQ curves, opto and FET compressor models, de-essing, tape-style saturation, and a filtered slap delay work isn't reinventing well-trodden primitives.
...
* Good, because `juce::dsp` provides ready building blocks (IIR/FIR filters, oversampling, convolution, state variable filters) that map directly onto this project's parallel bus routing, passive-style EQ curves, opto and FET compressor models, de-essing, tape-style saturation, and a filtered slap delay needs.
