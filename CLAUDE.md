...
## Status (v0.5.0 — Circuit Engines)
v2 replaced v1's topology entirely: v1 processed the "direct bus" by default and treated it as one of four equal faded busses; v2 makes the direct path a bit-transparent "channel" that always sums at unity and feeds all four return busses (unity taps), with every optional direct-path section off by default. See `docs/design-brief.md` for the binding v2 specification (topology, parameters, tests) and `docs/research-notes.md` for the sourced findings the voicing is derived from. v0.3.0 added the suite-wide M2 preset system (`src/presets/`, ported verbatim from `basilica-audio/nave`'s pilot implementation - see nave's `docs/preset-system-notes.md` for the replication recipe), 10 factory presets (13 today, `docs/presets.md`), and a German frame-string localisation (`resources/i18n/de.txt`). v0.4.0 was the M2 voicing pass (CRUSH's program-dependent colour). v0.5.0 replaced the hand-authored detectors with circuit engines (feedback-FET CRUSH, opto-carrier SANDWICH with T4B physics, residual-form ADAA, decramped/matched EQ rendering, tape transport on SLAP, 3 ms route-gain ramps) — all still zero-latency; post-release the PassiveEq degenerate-ladder Nyquist pole was fixed so every bus rests at exact zero after silence (#15). 233 Catch2 tests green.
...
- **`AudioBuffer::clear()` vs cached write pointers (JUCE 8.0.14):** in tests/tools, rebuild the `AudioBlock` per process call the way `processBlock` does. Writing through pointers cached from a hoisted `AudioBlock` never clears `AudioBuffer`'s internal `isClear` flag, so once it is set, every later `clear()` silently no-ops and a filter under test eats its own previous output — a self-amplifying loop that convincingly imitates an unstable filter (cost a full debugging detour in the #15 investigation).
- **Degenerate-order analog prototypes:** never run a reduced-order transfer function through a fixed higher-order bilinear transform — the extra pole/zero pair lands exactly on z = −1 and float coefficients don't cancel it (PassiveEq LF ladder, #15; the ConsoleEq iron integrator dodged the same pole by going impulse-invariant). Branch on the degenerate case and transform at true order.
