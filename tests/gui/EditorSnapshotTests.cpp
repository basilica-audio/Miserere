...
namespace
{
    ...
    void configureLiveLookingState (MiserereAudioProcessor& processor, MiserereAudioProcessorEditor& editor)
    {
        if (auto* channel = findDialByTitle (editor, "Channel compressor gain reduction meter"))
            channel->setImmediateDbForPreview (-4.0f);

        if (auto* bus = findDialByTitle (editor, "Bus gain reduction meter"))
            bus->setImmediateDbForPreview (-7.0f);

        const std::pair<const char*, double> knobPoses[] = {
            { "in_trim", 0.55 }, { "direct_fet_threshold", 0.35 }, { "sand_peakred", 0.6 },
            { "crush_input", 0.7 }, { "spread_width", 0.8 }, { "slap_tone", 0.4 },
            // Fader poses - SpriteFader is a juce::Slider, so the same
            // seeding path applies; visibly different cap positions.
            { "crush_level", 0.75 }, { "sand_level", 0.55 }, { "spread_level", 0.3 }, { "slap_level", 0.62 },
        };
        ...
        if (auto* fetEnabled = processor.apvts.getParameter ("direct_fet_enabled"))
            fetEnabled->setValueNotifyingHost (1.0f);
    }
}
...
TEST_CASE ("The editor renders a non-blank snapshot and writes the committed preview", "[gui][preview]")
{
    ...
    const auto outFile = juce::File (MSRR_REPO_ROOT).getChildFile ("docs").getChildFile ("gui-preview.png");
...
TEST_CASE ("The two VU needles render visibly different poses at different readings", "[gui][metering]")
{
    ...
    auto* input = findDialByTitle (editor, "Channel compressor gain reduction meter");
    auto* output = findDialByTitle (editor, "Bus gain reduction meter");
...
TEST_CASE ("Plate typography brightens the label zones over the raw plate", "[gui][a11y]")
{
    ...
    const auto plate = juce::ImageCache::getFromMemory (BinaryData::plate_miserere_png,
                                                        BinaryData::plate_miserere_pngSize);
    REQUIRE (plate.isValid());

    const auto manifest = MiserereAudioProcessorEditor::parseLayoutManifest();
    ...
    for (const auto& label : manifest.labels)
    {
        ...
        const auto snapshotArea = juce::Rectangle<int> ((int) label.cx - w / 2,
                                                        (int) label.cy - (int) label.h / 2
                                                            + MiserereAudioProcessorEditor::topStripHeight1x,
                                                        w, (int) label.h + 4);
        ...
        const auto plateArea1x = snapshotArea.translated (0, -MiserereAudioProcessorEditor::topStripHeight1x);
